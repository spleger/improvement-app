...
export const SettingsSchema = z.object({
    ...
    preferredDifficulty: z.number().int().min(1).max(10).optional(),
    challengesPerDay: z.number().int().min(1).max(10).optional(),
    realityShiftEnabled: z.boolean().optional(),
    preferredChallengeTime: z.string().max(50).optional(),
    focusAreas: z.array(z.string().max(50)).optional(),
    avoidAreas: z.array(z.string().max(50)).optional(),
    aiPersonality: z.enum(['empathetic', 'tough_love', 'scientific', 'casual']).optional(),
    includeScientificBasis: z.boolean().optional(),
    challengeLengthPreference: z.enum(['quick', 'medium', 'long']).optional(),
    notificationsEnabled: z.boolean().optional(),
    dailyReminderTime: z.string().max(10).optional(),
    streakReminders: z.boolean().optional(),
    theme: z.string().max(20).optional(),
    voiceId: z.enum(VALID_VOICE_IDS).optional(),
    aiCustomName: z.string().max(50).optional(),
    tonePreference: z.enum(['professional', 'friendly', 'casual']).optional(),
    rudeMode: z.boolean().optional(),
}).passthrough();
