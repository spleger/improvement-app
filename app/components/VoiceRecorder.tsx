...
type RecordingState = 'idle' | 'recording' | 'paused' | 'transcribing' | 'stopped' | 'processing' | 'saved';
...
export default function VoiceRecorder({ onSave }: VoiceRecorderProps) {
    ...
    const transcribeAudio = async () => {
        // Attempt server-side transcription with recorded audio
        try {
            const blob = new Blob(chunksRef.current, { type: 'audio/webm' });
            ...
            console.error('Failed server-side transcription', e);
        }
    };
    ...
    const stopRecording = () => {
        if (mediaRecorderRef.current) {
            ...
            const hadInterim = interimTranscript;
            if (interimTranscript) {
                setTranscript(prev => prev + interimTranscript + ' ');
                setInterimTranscript('');
            }

            // Immediately attempt server-side transcription if we have
            // little or no transcript from the Web Speech API
            const currentTranscript = transcript + (hadInterim || '');
            if (!currentTranscript || currentTranscript.trim().length < 10) {
                setState('transcribing');
                // Small delay to let MediaRecorder.onstop fire and finalize chunks
                setTimeout(async () => {
    ...
    const handleSave = async () => {
        ...
        setIsSaving(true);
        try {
            const finalTranscript = transcript.trim() || 'No transcript captured';
    ...
    return (
        <div className="voice-recorder">
            ...
            {/* Transcribing State - shimmer while generating transcript */}
            {state === 'transcribing' && (
                <div className="card p-lg">
                    <h3 className="heading-4 mb-md text-center">Generating Transcript...</h3>
                    ...
                    <div className="mb-lg">
                        ...
                        <div className="form-input w-full h-32 overflow-hidden" style={{ position: 'relative' }}>
                    ...
                    <div className="flex items-center justify-center gap-sm text-muted text-sm">
                        <Loader2 size={18} className="animate-spin" />
                        <span>Transcribing your recording with AI...</span>
                    </div>
                </div>
            )}
            ...
            {/* Processing / Saving State (FB-050: animated spin + breathe) */}
            {state === 'processing' && (
                <div className="card text-center p-xl save-breathing">
                    <div className="save-spinner-container">
                        <Loader2 size={48} className="animate-spin text-primary" />
                    </div>
                    <h3 className="heading-4 mb-sm">Saving your entry...</h3>
                    <p className="text-muted text-sm">Analyzing with AI</p>
                </div>
            )}
            ...
            {state === 'saved' && (
                <div className="card text-center p-xl">
                    <div className="text-5xl mb-md">Done</div>
