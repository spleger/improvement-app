-- PostgreSQL Schema for Goal Transformation App
...
CREATE TABLE IF NOT EXISTS "Goal" (
    ...
    "domainId" INTEGER REFERENCES "GoalDomain"("id") ON DELETE SET NULL,
    "title" TEXT NOT NULL,
    "description" TEXT,
    "currentState" TEXT,
    "desiredState" TEXT,
    ...
    "realityShiftEnabled" BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS "ChallengeTemplate" (
    ...
    "isRealityShift" BOOLEAN NOT NULL DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS "Challenge" (
    ...
    "userId" TEXT NOT NULL REFERENCES "User"("id") ON DELETE CASCADE,
    "templateId" TEXT REFERENCES "ChallengeTemplate"("id") ON DELETE SET NULL,
    "title" TEXT NOT NULL,
    "description" TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS "DiaryEntry" (
    ...
    "challengeId" TEXT REFERENCES "Challenge"("id") ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS "ProgressSnapshot" (
    ...
    "avgDifficultyFelt" DOUBLE PRECISION,
...
-- Seed Goal Domains
INSERT INTO "GoalDomain" ("id", "name", "icon", "color", "description", "examples") VALUES
...
-- Seed Demo User
INSERT INTO "User" ("id", "email", "passwordHash", "displayName", "themePreference", "onboardingCompleted", "timezone")
VALUES ('demo-user-001', 'demo@example.com', 'demo', 'Demo User', 'minimal', TRUE, 'UTC')
ON CONFLICT DO NOTHING;

-- Seed Challenge Templates
INSERT INTO "ChallengeTemplate" ("id", "domainId", "title", "description", "instructions", "durationMinutes", "difficulty", "isRealityShift", "scientificReferences", "tags", "successCriteria") VALUES
('tpl-lang-001', 1, 'Immersion Hour', 'Change all your device languages to your target language for one hour and navigate normally', 'Go to settings on your phone and computer. Change the language. Use your devices normally for the next hour.', 60, 3, FALSE, '["Immersive language exposure increases retention by 40%"]', '["immersion", "passive", "beginner-friendly"]', 'Complete 1 hour with devices in target language'),
('tpl-lang-002', 1, 'Think in Your Target Language', 'For 20 minutes, narrate your thoughts internally in your target language only', 'Set a timer for 20 minutes. Think about your day, plans, or surroundings entirely in the target language.', 20, 5, FALSE, '["Internal monologue practice builds automatic retrieval"]', '["thinking", "internal", "intermediate"]', 'Maintain internal dialogue for 20 minutes'),
('tpl-lang-003', 1, 'Stranger Conversation', 'Find a native speaker online or in person and have a 5-minute conversation', 'Use language exchange apps, local meetups, or online platforms to find a conversation partner.', 30, 7, TRUE, '["Active production with feedback accelerates fluency by 60%"]', '["speaking", "social", "advanced"]', 'Complete a 5+ minute conversation with a native speaker'),
('tpl-mob-001', 2, 'Morning Mobility Flow', 'Complete a 15-minute mobility routine as the first thing after waking up', 'Include: hip circles, shoulder rotations, cat-cow stretches, and gentle twists. Move slowly and breathe deeply.', 15, 3, FALSE, '["Morning mobility increases flexibility gains by 25%"]', '["morning", "routine", "beginner"]', 'Complete full 15-minute mobility routine before any other activity'),
('tpl-mob-002', 2, 'Deep Squat Hold', 'Accumulate 5 minutes in a deep squat position throughout the day', 'Hold a deep squat (heels down if possible) for as long as comfortable. Track total time.', 5, 5, FALSE, '["Deep squat practice restores natural hip mobility"]', '["squat", "flexibility", "intermediate"]', 'Total of 5 minutes accumulated in deep squat'),
('tpl-emo-001', 3, 'Emotion Labeling', 'Set 5 random alarms. When they go off, identify and write down your exact emotion', 'Use specific emotion words (not just good or bad). Examples: anxious, content, frustrated, curious.', 30, 3, FALSE, '["Emotion labeling reduces amygdala reactivity by 50%"]', '["awareness", "mindfulness", "beginner"]', 'Complete all 5 emotion check-ins with specific labels'),
('tpl-tol-001', 6, 'Cold Shower Finish', 'End your shower with 30 seconds of cold water', 'Take your normal shower, then switch to cold for the last 30 seconds. Breathe slowly and stay calm.', 1, 4, FALSE, '["Cold exposure increases dopamine by 250% and builds mental resilience"]', '["cold", "discomfort", "beginner"]', 'Complete 30 seconds of cold water at end of shower'),
('tpl-hab-001', 8, 'Habit Stacking', 'Attach your new habit to an existing routine you never skip', 'Identify a habit you do daily (brushing teeth, coffee). Add your new habit immediately before or after.', 10, 2, FALSE, '["Habit stacking increases habit formation success by 73%"]', '["habit", "routine", "beginner"]', 'Successfully stack new habit with existing routine')
ON CONFLICT DO NOTHING;
